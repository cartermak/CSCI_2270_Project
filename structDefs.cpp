...
struct Part {
    ...
    MachinePart *machines[NUM_OF_MACHINES];  // Array of MachinePart pointers to
                                             // hold this part on each machine
};
...
struct MachinePart {
    ...
    string notes;  // Notes field for the part (e.g. "This screw has screwed me
                   // so many times")
...
struct Replacement {
    ...
    string notes;  // Notes on replacement (e.g. "This screw was so fucked when
                   // I pulled it out, idk how nobody wasn't killed by a bowling
                   // pin")
};
